import json
import logging
from typing import List

import anthropic

from config import ANTHROPIC_API_KEY
from models.block import Block
from services.llm.base import LLMProvider

logger = logging.getLogger(__name__)


DEFAULT_TAGGING_MODEL = "claude-haiku-4-5-20251001"
DEFAULT_SUMMARY_MODEL = "claude-sonnet-4-20250514"


TAGGING_SYSTEM_PROMPT = """당신은 회의 전사 텍스트의 중요도를 분류하는 전문가입니다.
각 블록에 다음 4단계 중 하나를 부여하세요:

- high: 핵심 결정사항, 액션 아이템, 중요 논의 포인트
- medium: 관련 맥락, 배경 설명, 보조 논의
- low: 부가적 내용, 간접 관련 사항
- lowest: 사담, 인사말, 잡담, 주제 이탈

JSON 배열로 응답하세요. 다른 텍스트는 포함하지 마세요."""


SUMMARY_SYSTEM_PROMPT = """당신은 회의 전사 텍스트를 구조화된 회의록으로 정리하는 전문가입니다.
아래 템플릿 구조를 정확히 따르세요.

## 핵심 요약 섹션 작성 규칙:
1. 응답 맨 앞에 "## 핵심 요약" 섹션을 생성합니다 ("## 주요 논의 사항 & F/U 필요 요소"보다 먼저).
2. 2~4개의 bullet로 작성하며, 결정된 사항 > 방향 전환 > 기한이 있는 핵심 과제 순의 우선순위로 선별합니다.
3. 논의 경과 나열은 금지합니다 — "무엇이 정해졌고 무엇이 바뀌었는가"만 담습니다.

## 주요 논의 사항 & F/U 필요 요소 섹션 작성 규칙:
1. 전사 내용에서 주요 주제를 식별하여 번호를 매깁니다
2. 각 주제 안에서 "주요 논의"와 "F/U 필요 사항"을 분리합니다
3. F/U 항목 형식은 "- [{담당자이름}님] 할 일 (~{기한})" 입니다.
   - 담당자는 회의 참여자 명단에서만 선택하고, 이름만 사용합니다(직책 제외).
   - 같은 주제 내에서 같은 담당자의 F/U 항목들은 서로 인접하게 배치합니다.
   - 담당자가 명시되지 않은 경우 "- {할 일}" 형식으로 출력합니다. "(담당자 미명시)" 같은 접두어·표식은 절대 출력하지 마세요.
4. 기한은 전사에 명시적으로 언급된 날짜만 사용합니다. "다음 주 월요일" 같은 상대 표현을 특정 날짜로 계산·추정하지 마세요 (상대 표현은 그대로 두거나, 회의 날짜 기준이 전사에 명시된 경우에만 변환).
5. 주제에 속하지 않는 부가 내용은 "기타 메모"에 포함합니다

응답 시 "## 회의 개요" 섹션은 생성하지 마세요 (메타데이터에서 자동 삽입됩니다).
마지막에 Keywords 줄을 추가하세요."""


VALID_IMPORTANCE = {"high", "medium", "low", "lowest"}


class ClaudeProvider(LLMProvider):
    """Anthropic Claude provider.

    config keys:
      - api_key: anthropic key. fallback to ANTHROPIC_API_KEY env.
      - tagging_model: model id for tag(). fallback to DEFAULT_TAGGING_MODEL.
      - summary_model: model id for summarize(). fallback to DEFAULT_SUMMARY_MODEL.
    """

    def __init__(self, config: dict | None = None):
        self.config = config or {}
        self._client: anthropic.Anthropic | None = None

    def _get_client(self) -> anthropic.Anthropic:
        if self._client is None:
            key = (self.config.get("api_key") or "").strip() or ANTHROPIC_API_KEY
            self._client = anthropic.Anthropic(api_key=key)
        return self._client

    def tag(
        self,
        blocks: List[Block],
        title: str = "",
        participants: List[str] | None = None,
    ) -> dict[str, str]:
        if not blocks:
            return {}

        blocks_text = "\n".join(f"[{b.block_id}] {b.text}" for b in blocks)
        participants_str = ", ".join(participants) if participants else "미입력"

        user_prompt = f"""회의 제목: {title}
참여자: {participants_str}

전사 블록:
{blocks_text}

응답 형식:
[
  {{"block_id": "blk_001", "importance": "medium"}},
  ...
]"""

        model = (self.config.get("tagging_model") or "").strip() or DEFAULT_TAGGING_MODEL
        client = self._get_client()
        response = client.messages.create(
            model=model,
            max_tokens=4096,
            system=[{
                "type": "text",
                "text": TAGGING_SYSTEM_PROMPT,
                "cache_control": {"type": "ephemeral"},
            }],
            messages=[{"role": "user", "content": user_prompt}],
        )

        result_text = response.content[0].text.strip()

        try:
            tags = json.loads(result_text)
        except json.JSONDecodeError:
            start = result_text.find("[")
            end = result_text.rfind("]") + 1
            if start >= 0 and end > start:
                try:
                    tags = json.loads(result_text[start:end])
                except json.JSONDecodeError:
                    logger.warning(f"[AI Tagging] JSON parse failed: {result_text[:200]}")
                    return {}
            else:
                logger.warning(f"[AI Tagging] No JSON array found: {result_text[:200]}")
                return {}

        return {
            item["block_id"]: item["importance"]
            for item in tags
            if isinstance(item, dict)
            and "block_id" in item
            and "importance" in item
            and item["importance"] in VALID_IMPORTANCE
        }

    def summarize(
        self,
        blocks: List[Block],
        title: str = "",
        participants: List[str] | None = None,
        date: str = "",
    ) -> str:
        filtered = [b for b in blocks if b.importance in ("high", "medium")]
        if not filtered:
            filtered = blocks

        blocks_text = "\n".join(
            f"[{b.timestamp_start:.0f}s] {b.text}" for b in filtered
        )

        participants_str = ", ".join(participants) if participants else "미입력"

        user_prompt = f"""회의 제목: {title}
참여자: {participants_str}
회의 날짜: {date}

전사 텍스트 (중요도 상+중 블록만):
{blocks_text}

---
다음 형식으로 응답하세요:

## 핵심 요약

- (결정된 사항·방향 전환·기한이 있는 핵심 과제 중심 2~4개)
- ...

## 주요 논의 사항 & F/U 필요 요소

### 1. [주제명]
**주요 논의**
- ...

**F/U 필요 사항**
- [Sam님] 할 일 (~6/2)
- [Sam님] 다른 할 일
- [Jayden님] 또 다른 할 일 (~6/3)

### 2. [주제명]
...

---

## 기타 메모
- ...

Keywords: [키워드1, 키워드2, ...]"""

        model = (self.config.get("summary_model") or "").strip() or DEFAULT_SUMMARY_MODEL
        client = self._get_client()
        response = client.messages.create(
            model=model,
            max_tokens=8192,
            system=[{
                "type": "text",
                "text": SUMMARY_SYSTEM_PROMPT,
                "cache_control": {"type": "ephemeral"},
            }],
            messages=[{"role": "user", "content": user_prompt}],
        )

        return response.content[0].text.strip()
